...
namespace baseline_direct_native {
using direct_native::EfHitThrottle;
...
class FunctionalCache {
    ...
    llm_legacy_l1::PerSmL1Cache l1_;
    U max_lines_;
    ...
    void access(const CacheKey& key,bool write,std::uint8_t mask,std::uint8_t requested_mask,bool bypass,const Context& context) {
        ...
        const llm_legacy_l1::PerSmL1Access a{int(context.sm_id),key.matrix,key.line,write,-1,bypass,requested_mask};
...
public:
    FunctionalCache(const llm_legacy_l1::PerSmL1Config& l1,U l2_bytes,
        ...
        require(!options_.ef_fill_only||(l1.sector_validity&&!options_.skip_store_rfo&&!options_.bypass_streaming_reads),"EF fill-only diagnostic requires sector L1 and unchanged RFO/admission");
        require(options_.ef_hit_rate>=-1&&options_.ef_hit_rate<=65536,"EF hit rate out of range");
        require(options_.ef_hit_rate<0||(l1.sector_validity&&!options_.ef_fill_only&&!options_.skip_store_rfo&&!options_.bypass_streaming_reads),"EF hit calibration requires sector L1 and no simultaneous cache override");
