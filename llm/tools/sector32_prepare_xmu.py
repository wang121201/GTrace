...
ARMS = [('p32-old128',32,'old128','none',2),
        ('p32-sector32',32,'sector32','run-end',0),
        ('p128-sector32',128,'sector32','run-end',1),
        ('p128-phase-drain',128,'sector32','measured-phase-end',8)]
...
def main():
    ...
    prepared=[]
    for name,p,policy,drain,cpu in ARMS:
        old=originals[p]; out=cases/name; out.mkdir(exist_ok=False)
        argv=old['argv'][:]; argv[2]=str(producer); replace(argv,'--runner',binary)
        replace(argv,'--output',out/'preflight'); argv+=['--drain-policy',drain]
        caseenv=dict(old['environment'],TILEGEN_L2_DATA_POLICY=policy,TILEGEN_ADA_L1_PROFILE='r4')
        for k,v in {'TILEGEN_EF_HIT_RATE':'288','TILEGEN_L2_DIRTY_AGE_ACCESSES':'64000000','TILEGEN_ADA_REQUIRE_OBSERVED':'1'}.items():need(caseenv.get(k)==v,'controlled cache parameter changed')
        ...
        spec=dict(case_id=name+'-r1',tool='native-functional-sector32-L2-control',input_kind='COMPLETE_NATIVE_TILEGRAPH_SOURCE_PROGRAMS_AND_API_HISTORY',cpu=cpu,gpu=None,seconds=21600,rss_limit_bytes=16<<30,argv=argv,environment=caseenv,sources=allpins)
        sp=out/'spec.json';save(sp,spec)
        prepared.append(dict(case=name,prefill=p,decode=2,policy=policy,drain_policy=drain,cpu=cpu,spec=pin(sp),output=str(out/'result'),job_directory=str(out/'job'),admission=admissions[p]))
