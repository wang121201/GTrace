"""Read only this P128 task and export whitelisted scalar aggregates, never traces."""
import argparse
import collections
import datetime
import hashlib
...
STATUSES={'NOT_STARTED','PREFLIGHT_RUNNING','PASS_COMPLETE_NATIVE_INPUT_PREFLIGHT','RUNNING_COMPLETE_NATIVE_GRAPH_CACHE',
 ...
 'PASS_STREAMED_SOURCE_FUNCTIONAL_CACHE_RUN','PASS_P128_PAIRED_PREFLIGHT_NOT_EXECUTED'}
...
def ncu_reference(directory,decode):
    path=directory/'ncu-result.json';x=load(path)
    ...
    if finite!=('qwen25_1p5b',128,decode,1,'bfloat16',1) or contract!=expected:
        return dict(status='INPUT_CONTRACT_MISMATCH',ROI_rows=None,**identity)
    if x.get('metadata_actual_controls_compared') is not True or not str(x.get('status','')).startswith('PASS_CLOSED_RAW_NCU_'):
        return dict(status='REFERENCE_NOT_CLOSED_OR_UNPAIRED',ROI_rows=None,**identity)
    rows=[]
    for r in x.get('ROI_rows',[]):
        roi=r.get('roi')
...
def aggregate_variant(directory,case_id,decode,profile):
    out=directory/profile;job=directory/(profile+'-job')
    row=dict(case_id=case_id,model_key='qwen25_1p5b',prefill_length=128,decode_steps=decode,profile=profile,status='NOT_STARTED',completed_phases=[],NCU=ncu_reference(directory,decode))
    ...
    if preparation:
        ...
        for field in ('graph','launch_resources'):
...
def aggregate(root):
    ...
    for receipt in (root/'cases').glob('*/prepare-result.json'):
        p=load(receipt)
        if not p or p.get('schema')!='P128_PAIRED_PREPARATION_V1':continue
        d=p.get('decode_steps');case_id=p.get('case_id')
        if d not in DECODE_STEPS or p.get('model_key')!='qwen25_1p5b' or p.get('prefill_length')!=128:continue
        ...
        if d in known:raise ValueError('duplicate prepared decode length')
        known[d]=(receipt.parent,case_id)
    cases=[]
    for d in DECODE_STEPS:
        directory,case_id=known.get(d,(root/'cases'/('qwen-p128d'+str(d)),'qwen-p128d'+str(d)))
        for profile in ('r2','r4'):cases.append(aggregate_variant(directory,case_id,d,profile))
    return dict(schema='P128_LLM_R2_R4_SAFE_AGGREGATES_V1',generated_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),scope='safe aggregate snapshot; no raw addresses/symbols; not full acceptance',cases=cases)
