...
// One process-fixed policy parameter; no phase, tensor, address or NCU input.
...
inline direct_native::DiagnosticOptions options(){direct_native::DiagnosticOptions o;o.ef_hit_rate=selected_ef_hit_rate();o.dirty_age_accesses=selected_dirty_age();return o;}
inline J configuration(){return {{"L1",llm_l1::description(l1_config(),llm_l1::assumed_shared(),llm_l1::initial_origin())},{"L2",{{"bytes",41943040},{"geometry","PAPER_ADA_L2_V1_20x1024x16"},{"read_fill_RFO_bytes",128},{"writeback_request_bytes",32},{"EF_hit_numerator",selected_ef_hit_rate()},{"EF_hit_denominator",65536},{"skip_store_RFO",false},{"dirty_age_accesses",selected_dirty_age()},{"dirty_age_policy","LAST_STORE_L2_ACCESS_BUDGET_KEEP_CLEAN_TAG_LRU_UNCHANGED"}}},{"sm_policy","MODELED_CTA_LINEAR_ID_MOD_48_NOT_OBSERVED_SM"},{"issue_order","SERIAL_CALLER_STREAM_ORDER_NOT_GPU_TIMING"},{"DMA_model","L2_COHERENT_FUNCTIONAL_128B_CHUNKS_NOT_HARDWARE_QUALIFIED"},{"end_flush",false},{"cache_policy_modified",(selected_ef_hit_rate()!=288||selected_dirty_age()!=0)}};}
...
class Runner {
 ...
 U read_fills_=0,store_rfos_=0,atomic_rfos_=0,source_hash_=14695981039346656037ULL,post_hash_=14695981039346656037ULL;
 ...
 void invoke(const Effect& e,const Policy& policy,bool api_chunk){
  ...
  direct_native::Context ctx;ctx.call_index=ordinal_;ctx.node_id=id_;ctx.cta=e.cta;ctx.warp=e.warp;ctx.pc=e.pc;ctx.sm_id=api_chunk?0:e.cta%48;ctx.low_priority=policy.low_priority;ctx.role_id=policy.semantic=="weights"?1:policy.semantic=="activation"?2:policy.semantic=="kv"||policy.semantic=="kvcache"?3:4;
...
public:
 explicit Runner(std::ostream& out):out_(out){cache_=std::make_unique<direct_native::FunctionalCache>(l1_config(),40ULL<<20,[](int matrix,U a){need(matrix==0,"global VA namespace");return a;},[this](const native_trace::Record& r){
   need(r.bytes==(r.cause==native_trace::Cause::DirtyWriteback?32:128),"frozen transaction size changed");if(r.cause!=native_trace::Cause::DirtyWriteback)++(current_atomic_?atomic_rfos_:current_write_?store_rfos_:read_fills_);for(U n:native_trace::words(r))hash(post_hash_,n);
  },GTSim::L2GeometryConfig::paper_ada_l2_v1(),options());last_snapshot_=snapshot();}
 J dirty_distribution()const{J v=cache_->dirty_group_observation();v.erase("resident_dirty_lines_by_group");return v;}
 J snapshot()const{J s=cache_->snapshot();s["L1_adapter_observation"]=cache_->l1_observation();s["source_warp_events"]=memory_events_;s["zero_global_effect_events"]=zero_events_;s["source_kernel_payload_bytes_once"]=kernel_payload_;s["source_API_payload_bytes"]=api_payload_;s["source_read_effect_bytes"]=read_effect_;s["source_write_effect_bytes"]=write_effect_;s["source_atomic_RMW_events"]=atomic_events_;s["source_global_to_shared_events"]=g2s_events_;s["API_ranges"]=api_ranges_;s["API_128B_chunks"]=api_chunks_;s["kernel_boundaries"]=kernels_;s["API_boundaries_without_cache_flush"]=apis_;s["DRAM_load_fill_requests"]=read_fills_;s["DRAM_store_RFO_requests"]=store_rfos_;s["DRAM_atomic_RFO_requests"]=atomic_rfos_;s["DRAM_load_fill_bytes"]=read_fills_*128;s["DRAM_store_RFO_bytes"]=store_rfos_*128;s["DRAM_atomic_RFO_bytes"]=atomic_rfos_*128;s["source_effect_projection_fnv1a64"]=source_hash_;s["postcache_record_fnv1a64"]=post_hash_;s["dirty_tail_bytes"]=s.at("resident_dirty_sectors").get<U>()*32;
  need(s.at("dirty_sector_creations").get<U>()*32==s.at("DRAM_write_bytes").get<U>()+s.at("dirty_tail_bytes").get<U>(),"dirty byte conservation");need(s.at("DRAM_read_requests").get<U>()==read_fills_+store_rfos_+atomic_rfos_,"typed fill conservation");return s;}
 ...
 void command(const J& j){need(j.is_object()&&j.contains("type"),"command object/type required");auto type=j.at("type").get<std::string>();need(!ended_,"input after run_end");
  ...
  if(type=="snapshot"){need(!open_,"snapshot inside operation");J s=snapshot();auto now=std::clock();emit_summary({{"type","snapshot"},{"label",j.at("label")},{"cumulative",s},{"dirty_age_observation",cache_->dirty_age_observation()},{"dirty_group_observation",dirty_distribution()},{"delta_since_previous_snapshot",difference(last_snapshot_,s)},{"CPU_minutes_since_previous_snapshot",double(now-snapshot_cpu_)/CLOCKS_PER_SEC/60.0},{"CPU_minutes_since_run_start",double(now-cpu0_)/CLOCKS_PER_SEC/60.0},{"L2_flush",false}});last_snapshot_=s;snapshot_cpu_=now;return;}
  if(type=="run_end"){need(!open_,"run ended inside operation");cache_->verify_resident_ledger();ended_=true;emit_summary({{"type","run_end"},{"cumulative",snapshot()},{"L2_flush",false}});return;}
  throw std::invalid_argument("unknown stream command");
 }
 bool ended()const{return ended_;}
 J summary()const{need(started_&&ended_&&!open_,"input missing explicit run_end");J roles=J::object();for(const auto& [name,v]:roles_)roles[name]={{"read_effect_bytes",v[0]},{"write_effect_bytes",v[1]}};return {{"status","PASS_STREAMED_SOURCE_FUNCTIONAL_CACHE_RUN"},{"configuration",configuration()},{"snapshot",snapshot()},{"source_by_semantic",roles},{"EF_hit_throttle",cache_->ef_hit_throttle_observation()},{"dirty_age_observation",cache_->dirty_age_observation()},{"dirty_group_observation",dirty_distribution()},{"CPU_minutes",double(std::clock()-cpu0_)/CLOCKS_PER_SEC/60.0},{"wall_minutes",std::chrono::duration<double>(std::chrono::steady_clock::now()-wall0_).count()/60.0},{"expanded_source_records_retained",0},{"postcache_records_retained",0},{"actual_SM_or_issue_timing_claimed",false},{"no_end_flush",true},{"whole_graph_input_qualification_owned_by_caller",true}};}
};
}
