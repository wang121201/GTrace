...
template<class Sink>void generate(const J& command,const Sink& sink){
 ...
 need(b.at("schema")=="CURRENT_QWEN_GEMV_SOURCE_BINDING_V1","Qwen finite binding schema");U variant=number(b.at("variant"),6,7),tx=number(b.at("thread_x")),K=number(b.at("K")),N=number(b.at("N")),lda=number(b.at("lda"));need(lda==K,"GEMV contiguous lda");need((variant==6&&tx==16&&K==1536&&(N==1536||N==2048||N==17920||N==151936)&&code=="a67f8d265deb5e1402037e2c45c1a28c7dc5c343e97e493dd24f0727b4ac50ed")||(variant==7&&tx==32&&K==8960&&N==1536&&code=="31af276ca5f2a53c3d67f65622dcee3ec8662093c79ce0dd6fed10ce5acd00d5"),"finite Qwen shape/code/thread specialization");need(b.at("grid")==J({N/4,1,1})&&b.at("block")==J({tx,4,1}),"Qwen geometry");U W=number(b.at("weights"),1),X=number(b.at("activation"),1),Y=number(b.at("output"),1),bias=number(b.at("bias")),flags=number(b.at("epilog_flags"));need((flags==1&&bias==0)||(flags==4&&bias>0&&N==2048),"Qwen epilog bias contract");need(W%2==0&&X%2==0&&Y%2==0&&W<=UINT64_MAX-N*K*2&&X<=UINT64_MAX-K*2&&Y<=UINT64_MAX-N*2&&(!bias||(bias%2==0&&bias<=UINT64_MAX-N*2)),"GEMV global root extents");
 ...
 std::vector<U> ctas;if(command.contains("ctas")){need(!command.contains("cta_range"),"choose ctalist or range");for(const auto& x:command.at("ctas"))ctas.push_back(number(x,0,N/4-1));std::set<U> unique(ctas.begin(),ctas.end());need(unique.size()==ctas.size(),"duplicate selected CTA");}else{const auto& range=command.at("cta_range");need(range.is_array()&&range.size()==2,"explicit CTA half-open range");U lo=number(range[0],0,N/4),hi=number(range[1],0,N/4);need(lo<=hi,"CTA range reversed");for(U cta=lo;cta<hi;++cta)ctas.push_back(cta);}
 need(!ctas.empty(),"empty GEMV CTA program");
 for(U cta:ctas)for(U warp=0;warp<tx*4/32;++warp){
  ...
  for(U outer=0;outer<K/tx;outer+=64){U remaining=std::min<U>(64,K/tx-outer),consumed=0;for(unsigned s=0;s<4;++s)while(remaining>=widths[s]){
    for(const auto& step:stages[s]){e.pc=step.pc;U column=outer+consumed+step.index;for(U lane=0;lane<32;++lane){U tid=warp*32+lane,output_row=cta*4+tid/tx;e.addresses[lane]=(step.weights?W:X)+2*((step.weights?output_row*lda:0)+tid%tx+column*tx);}sink(e,step.weights?pw:px);}
  ...
  if(flags==4){e.operation="READ";e.pc=number(c.at("epilog")[0]);for(U lane=0;lane<32;++lane)if(e.global_mask&(U{1}<<lane))e.addresses[lane]=bias+2*(cta*4+(warp*32+lane)/tx);sink(e,pb);}
  e.operation="WRITE";e.pc=number(c.at("epilog")[1]);for(U lane=0;lane<32;++lane)if(e.global_mask&(U{1}<<lane))e.addresses[lane]=Y+2*(cta*4+(warp*32+lane)/tx);sink(e,py);
 }
}
