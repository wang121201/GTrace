#!/usr/bin/env python3
"""Host argument capture: unchanged native workload, exact sealed static plan."""
import argparse
import fcntl
import json
import os
...
from support import HERE, TASK, R1_SHA, CAPTURE_SHA, R2_SHA, identity, load_r1, need, sha
...
def main():
    ...
    p.add_argument('--run-name', default='arguments-r1')
    ...
    plan = dict(schema='SGLANG_PD_NATIVE_ARGUMENT_PLAN_V1', output=str(TASK / 'runs' / a.run_name),
                observer=str(HERE / 'build/observer.so'), metadata_cap_bytes=1 << 30,
                workload_manifest_sha256=CAPTURE_SHA, metadata_dependency_manifest_sha256=R1_SHA,
                argument_validator='consumer.py in the same sealed package',
    ...
    own = identity(HERE)
    import consumer
    plan_path = HERE / 'argument-plan.json'
    reference = json.loads((HERE / 'manifest.json').read_text())['reference_observer']
    source_root = TASK / 'runs' / 'observer-r2'
    source_controller = json.loads((source_root / 'controller.json').read_text())
    need(identity(TASK / 'observer-r2', R2_SHA)['manifest_sha256'] == R2_SHA, 'frozen r2 source identity')
    need(source_controller['status'] == 'PASS_NATIVE_METADATA_CENSUS_ONLY' and
         sha(source_root / 'controller.json') == reference['controller_sha256'] and
         sha(source_root / 'native-census.json') == reference['census_sha256'] == source_controller['native_census_sha256'],
         'closed exact source observer census required')
    r1 = load_r1()
    ...
    receipt = dict(plan, status='PREPARING', start=base.now(), package=own, workload_package=package,
                   metadata_dependency=validator, build=build, argument_plan_sha256=sha(plan_path), reference_observer=reference)
    fds = []
    def stop(sig, frame):
        raise InterruptedError('native arguments signal ' + str(sig))
    ...
    try:
        ...
        census = consumer.validate_arguments(step, c, observer_parent=out, plan_path=plan_path)
        need(census['status'] == 'PASS_NATIVE_ARGUMENT_PAYLOADS_ONLY', 'full argument payload closure required')
        ...
        # Publish the qualified artifact only after all post-run checks pass.
        base.write(out / 'argument-census.json', census)
        receipt.update(status='PASS_NATIVE_ARGUMENT_PAYLOADS_ONLY', argument_census_sha256=sha(out / 'argument-census.json'),
        ...
        receipt['host_timing'] = dict(
            elapsed_seconds=time.monotonic()-wall_start,
            controller_CPU_seconds=(own_cpu.ru_utime-cpu_start.ru_utime)+(own_cpu.ru_stime-cpu_start.ru_stime),
            waited_child_CPU_seconds=(child_cpu.ru_utime-child_cpu_start.ru_utime)+(child_cpu.ru_stime-child_cpu_start.ru_stime),
            definition='process user+system since controller main entry; waited child usage includes child-accounted descendants; not GPU time')
        base.write(out / 'controller.json', receipt)
        for fd in reversed(fds):
            os.close(fd)
    print(json.dumps(dict(status=receipt['status'], receipt=str(out / 'controller.json'))))
    return 0 if receipt['status'] == 'PASS_NATIVE_ARGUMENT_PAYLOADS_ONLY' else 1
