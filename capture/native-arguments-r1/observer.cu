// Host-argument derivative of the pinned HyFiSS NVBit callback/static-inspection
...
namespace sgobs {
using std::string;
const uint64_t DEFAULT_CAP = 1ull << 30;
const uint64_t RESERVED_FINISH = 128ull << 10;
const uint64_t MAX_LAUNCHES = 100000, MAX_FUNCTIONS = 4096;
...
struct State {
  ...
  File launch_file,scope_file,function_file,instruction_file,lifecycle_file,allocation_file,argument_file;
  sgargs::Ledger argument_ledger{sgargs::make_plan(),sgargs::make_limits()};
  ...
  void initialize() {
    ...
    need(cap>=(2ull<<20)&&cap<=DEFAULT_CAP,"metadata quota outside 2..1024 MiB envelope");
    ...
    launch_file.open(dir,"launch-journal.jsonl");scope_file.open(dir,"scope-journal.jsonl");function_file.open(dir,"functions.jsonl");instruction_file.open(dir,"static-instructions.jsonl");lifecycle_file.open(dir,"lifecycle.jsonl");allocation_file.open(dir,"allocation-journal.jsonl");argument_file.open(dir,"launch-arguments.jsonl");enabled=true;
    emit(lifecycle_file,prefix("observer_init")+",\"nvbit_version\":"+quote(NVBIT_VERSION)+",\"nvbit_cuda_header_version\":"+number(CUDA_VERSION)+",\"dynamic_instrumentation\":false,\"code_hash_kind\":\"sha256_nvbit_decoded_instruction_rows_v1\",\"module_binary_hash_available\":false,\"max_total_bytes\":"+number(cap)+"}\n");
  }
...
Function describe(CUcontext ctx,CUfunction f) {
  ...
  for(auto ins:instructions) {
    ...
    s.emit(s.instruction_file,"{\"schema\":\"sg_nvbit_static_instruction_v1\",\"function_id\":"+number(d.id)+",\"instruction\":"+canonical.substr(0,canonical.size()-1)+"}\n");
  }
...
string launch_body(CUcontext ctx,nvbit_api_cuda_t cbid,void *params,uint64_t native_id,const string& api) {
  State &s=state();need(params,"launch params null");CUfunction f=nullptr;CUstream stream=nullptr;
  unsigned gx=0,gy=0,gz=0,bx=0,by=0,bz=0,shared=0;string attrs="[]";void**kernel_params=nullptr;void**extra=nullptr;
  if(cbid==API_CUDA_cuLaunchKernel) {
    const auto &p=*static_cast<const cuLaunchKernel_params*>(params);f=p.f;stream=p.hStream;kernel_params=p.kernelParams;extra=p.extra;
    ...
    const auto &p=*static_cast<const cuLaunchKernel_ptsz_params*>(params);f=p.f;stream=p.hStream;kernel_params=p.kernelParams;extra=p.extra;
    gx=p.gridDimX;gy=p.gridDimY;gz=p.gridDimZ;bx=p.blockDimX;by=p.blockDimY;bz=p.blockDimZ;shared=p.sharedMemBytes;
  } else {
    const CUlaunchConfig *c=nullptr;
    if(cbid==API_CUDA_cuLaunchKernelEx){const auto &p=*static_cast<const cuLaunchKernelEx_params*>(params);f=p.f;c=p.config;kernel_params=p.kernelParams;extra=p.extra;}
    else if(cbid==API_CUDA_cuLaunchKernelEx_ptsz){const auto &p=*static_cast<const cuLaunchKernelEx_ptsz_params*>(params);f=p.f;c=p.config;kernel_params=p.kernelParams;extra=p.extra;}
    else throw std::runtime_error("argument unsupported launch callback API");
  ...
  if(inspected)need(rr==CUDA_SUCCESS&&sr==CUDA_SUCCESS&&lr==CUDA_SUCCESS&&nr>=0&&ss>=0&&ls>=0,"argument native resource query failed");
  const string argument_ref=capture_native_arguments(native_id,d,cid,ptr(stream),api,gx,gy,gz,bx,by,bz,shared,ss,nr,ls,attrs,kernel_params,extra);
  ...
  return ",\"metadata_supported\":true,\"context_id\":"+number(cid)+",\"context_handle_u64\":"+number(ptr(ctx))+",\"function_id\":"+number(d.id)+",\"related_function_ids\":"+uarray(ids)+",\"function_name\":"+quote(d.name)+",\"code_sha256\":"+code_json+",\"static_inspection_performed\":"+(inspected?"true":"false")+",\"code_sha256_kind\":\"sha256_nvbit_decoded_instruction_rows_v1\",\"module_handle_u64\":"+number(d.module)+",\"module_epoch\":"+number(d.module_epoch)+",\"module_binary_sha256\":null,\"stream_u64\":"+number(ptr(stream))+",\"grid\":"+dims(gx,gy,gz)+",\"block\":"+dims(bx,by,bz)+",\"dynamic_shared_bytes\":"+number(shared)+",\"static_shared_bytes\":"+(sr==CUDA_SUCCESS?signed_number(ss):"null")+",\"registers\":"+(rr==CUDA_SUCCESS?signed_number(nr):"null")+",\"local_bytes_per_thread\":"+(lr==CUDA_SUCCESS?signed_number(ls):"null")+",\"binary_version\":"+(br==CUDA_SUCCESS?signed_number(bv):"null")+",\"ptx_version\":"+(pr==CUDA_SUCCESS?signed_number(pv):"null")+",\"attribute_query_results\":["+signed_number(rr)+","+signed_number(sr)+","+signed_number(lr)+","+signed_number(br)+","+signed_number(pr)+"],\"launch_attributes\":"+attrs+",\"argument_sizes\":"+uarray(d.args)+",\"parameter_layout_sha256\":"+layout_json+",\"parameter_values_captured\":"+(argument_ref.empty()?"false":"true")+argument_ref;
}
...
void State::finish(const string &why) {
  ...
  bool pass=closed&&argument_ledger.closed()&&errors.empty()&&failed_launches==0&&unsupported==0&&graph_nodes==0&&unknown_attrs==0&&internal_dispatches.load()==0;
  string inventory="[";File *files[]={&launch_file,&scope_file,&function_file,&instruction_file,&lifecycle_file,&allocation_file,&argument_file};
  for(unsigned i=0;i<7;++i){if(i)inventory+=',';File &f=*files[i];inventory+="{\"name\":"+quote(f.name)+",\"bytes\":"+number(f.bytes)+",\"sha256\":"+quote(f.digest())+"}";}inventory+=']';
  ...
  string out="{\"schema\":\"sg_nvbit_observer_finish_v1\",\"status\":"+quote(pass?"PASS_NATIVE_ARGUMENT_OBSERVER_CLOSED_NOT_TRACE":"FAIL_OR_UNSUPPORTED_NATIVE_ARGUMENT_OBSERVER")+",\"reason\":"+quote(why)+",\"pid\":"+number(owner)+",\"start_ticks\":"+number(ticks)+",\"output\":"+quote(path)+",\"launch_before_count\":"+number(launches)+",\"launch_return_count\":"+number(returned)+",\"launch_error_count\":"+number(failed_launches)+",\"unsupported_dispatch_count\":"+number(unsupported)+",\"graph_node_callback_count\":"+number(graph_nodes)+",\"unknown_launch_attribute_count\":"+number(unknown_attrs)+",\"unbound_launch_count\":"+number(unbound_launches)+",\"context_count\":"+number(contexts.size())+",\"open_context_count\":"+number(open_contexts)+",\"epoch_begin_count\":"+number(epoch_begun)+",\"epoch_end_count\":"+number(epoch_ended)+",\"active_epoch\":"+number(active_epoch)+",\"function_count\":"+number(functions.size())+",\"observed_launch_thread_count\":"+number(launch_threads.size())+",\"observed_stream_count\":"+number(streams.size())+",\"marked_epoch_thread_count\":"+number(epoch_threads.size())+",\"marked_epoch_stream_count\":"+number(epoch_streams.size())+",\"internal_inspection_callback_count\":"+number(internal_callbacks.load())+",\"internal_inspection_dispatch_count\":"+number(internal_dispatches.load())+",\"metadata_bytes_before_finish\":"+number(total)+",\"max_total_bytes\":"+number(cap)+",\"api_counts\":"+apis+",\"errors\":"+es+",\"files\":"+inventory+",\"dynamic_instrumentation\":false,\"memory_addresses_captured\":false,\"actual_sm_placement_captured\":false,\"kernel_argument_values_captured\":true,\"kernel_argument_capture_scope\":\"sealed_measured_host_parameter_vectors_only\",\"device_memory_dereferenced\":false,\"typed_objects_or_relocation_qualified\":false,\"module_binary_hash_available\":false,\"static_code_hash_is_decoded_sass_not_cubin\":true,\"device_synchronization_inserted\":false,\"gpu_memory_allocated_by_observer\":false,\"native_callback_completeness_independently_proven\":false,\"template_or_gtsim_admission\":false}\n";
  out.pop_back();out.pop_back();out+=",\"argument_sideband_plan_sha256\":"+quote(sgargs::ARGUMENT_PLAN_SHA)+",\"argument_sideband_closed\":"+string(argument_ledger.closed()?"true":"false")+",\"argument_sideband_entries\":"+number(argument_ledger.entries())+",\"argument_sideband_returns\":"+number(argument_ledger.returns())+",\"argument_sideband_argument_count\":"+number(argument_ledger.arguments())+",\"argument_sideband_raw_bytes\":"+number(argument_ledger.raw_bytes());out+=",\"allocation_API_calls\":"+number(allocation_calls)+",\"unparsed_allocation_API_calls\":"+number(unparsed_allocation_calls)+",\"complete_allocator_coverage\":false}\n";
...
void nvbit_at_cuda_event(CUcontext ctx,int is_exit,nvbit_api_cuda_t cbid,const char *name,void *params,CUresult *status) {
  ...
  try {
    ...
    if(!is_exit) {
      ...
      string specific;
      if(direct(cbid)||extended(cbid))specific=launch_body(ctx,cbid,params,p.id,api);
      else {++s.unsupported;throw std::runtime_error("argument unsupported kernel dispatch API");}
      ...
      s.argument_ledger.complete(p.id,status&&*status==CUDA_SUCCESS);
