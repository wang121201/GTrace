"""Build the host argument observer in a fresh directory. No GPU execution.
...
def main():
    ...
    for name in sorted(manifest['build_inputs']):
    ...
    inputs=[pin(HERE/name) for name in sorted(manifest['build_inputs'])]+[pin(a.cuda/'bin/nvcc'),pin(a.nvbit/'libnvbit.a')]
    ...
    commands=[('host',[nvcc,'-dc','-c','-std=c++11','-I'+str(HERE),'-I'+str(a.nvbit),'-Xptxas','-cloning=no',
    ...
    result=dict(schema='SG_NVBIT_NATIVE_ARGUMENT_BUILD_V1',status='FAIL_BUILD_ONLY',GPU_run=False,
    ...
    try:
        for name,argv in commands:
            ...
            with (out/(name+'.stdout')).open('xb') as stdout,(out/(name+'.stderr')).open('xb') as stderr:
                ...
                try:code=active.wait(timeout=480)
                finally:
