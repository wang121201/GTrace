...
tiny_runtime::sp_event_mode=events;const auto initial=session.statistics();g::Cycle previous=0,sum_windows=0,sum_kernel=0,sum_drain=0;U sum_r=0,sum_w=0,prior_r=0,prior_w=0;tiny_sha::Sha256 workflow_digest;double engine_total=0;
 auto execute_one=[&](std::size_t index){memory.backend->set_trace_context(index);const auto& k=*prepared[index];std::cerr<<J({{"progress_started_call",index+1},{"progress_completed_calls",index},{"total_calls",prepared.size()},{"family",k.family},{"source_launch_key",k.call.at("source_launch_key")},{"total_CTAs",k.executed},{"continuous_cycle",session.cycle()}}).dump()<<std::endl;
  ...
  if(pb){p::need(pb->copy_done==2576*k.executed&&pb->zero_done==16*k.executed&&pb->tensor_work==33554432*k.executed,"P28 copy/tensor census");p::need(d.at("pre_l1_reads")==10240*k.executed&&d.at("l1_bypassed_transactions")==10240*k.executed&&d.at("l1_read_hits")==0&&d.at("l1_read_misses")==0,"P28 known bypass per-call delta");e["P28_node_audit"]=pb->receipt();}
  if(gb){const auto&m=*lease->nextmodel;p::need(gb->copy_done==m.copies*k.executed&&gb->zero_done==m.zero_copies*k.executed&&gb->tensor_work==m.tensors*2048*k.executed,"new GEMM per-CTA work");U lines=k.family=="GEMMGate"?20480:k.family=="GEMMDown"?35840:10240;p::need(d.at("pre_l1_reads")==lines*k.executed&&d.at("l1_bypassed_transactions")==lines*k.executed&&d.at("l1_read_hits")==0&&d.at("l1_read_misses")==0,"new GEMM known BYPASS counters");e["GEMM_node_audit"]=gb->receipt();}
  if(ab){const auto&m=*lease->attentionmodel;p::need(ab->copy_done==m.copies*k.executed&&ab->zero_done==m.zero_copies*k.executed&&ab->tensor_work==m.tensors*2048*k.executed,"Attention copy/shared/tensor work");p::need(d.at("pre_l1_reads")==m.read_lines*k.executed&&d.at("pre_l1_writes")==m.write_lines*k.executed&&d.at("l1_bypassed_transactions")==m.copy_lines*k.executed,"Attention ordinary global and known copy bypass census");e["Attention_node_audit"]=ab->receipt();}
