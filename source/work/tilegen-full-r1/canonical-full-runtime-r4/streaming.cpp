...
#include "source_native_summary.h"
#include "ef_natural_control.h"
...
namespace native_writer_control { inline bool natural_d1=false,full_d2=false; inline bool active(){return natural_d1||full_d2;} }
namespace shared_cache_control {
inline bool enabled=false;inline unsigned numerator=288;
using J=nlohmann::json;using U=std::uint64_t;
inline J counters(const GTSim::PerSmL1Statistics&s){return {{"read_sector_requests",s.read_sector_requests},{"read_sector_hits",s.read_sector_hits},{"read_sector_misses",s.read_sector_misses},{"write_sector_requests",s.write_sector_requests},{"write_sector_hits",s.write_sector_hits},{"write_sector_misses",s.write_sector_misses},{"forwarded_read_sector_requests",s.forwarded_read_sector_requests},{"forwarded_write_sector_requests",s.forwarded_write_sector_requests},{"bypassed_read_sector_requests",s.bypassed_read_sector_requests},{"bypassed_write_sector_requests",s.bypassed_write_sector_requests},{"pre_l1_transactions",s.pre_l1_transactions},{"pre_l1_reads",s.pre_l1_reads},{"pre_l1_writes",s.pre_l1_writes},{"bypassed_transactions",s.bypassed_transactions},{"read_hits",s.read_hits},{"read_misses",s.read_misses},{"write_hits",s.write_hits},{"write_misses",s.write_misses},{"filtered_reads",s.filtered_reads},{"l2_input_transactions",s.l2_input_transactions}};}
inline J readiness(const GTSim::ReadinessStatistics&s){return {{"issued_tickets",s.issued_tickets},{"completed_tickets",s.completed_tickets},{"stale_tickets",s.stale_tickets},{"ready_transitions",s.ready_transitions},{"pending_tag_reads",s.pending_tag_reads},{"write_hits_on_pending",s.write_hits_on_pending},{"live_tickets",s.live_tickets},{"peak_live_tickets",s.peak_live_tickets},{"occupied_lines",s.occupied_lines},{"ready_lines",s.ready_lines},{"pending_lines",s.pending_lines}};}
inline J delta(const J&after,const J&before){J out=J::object();for(const auto&x:after.items()){
 const U a=x.value().get<U>(),b=before.at(x.key()).get<U>();if(a<b)throw std::logic_error("sector L1 counter decreased");out[x.key()]=a-b;}return out;}
}
...
J run(const J& in,compressed_frame::Cache&frames,bool validate,bool eager,bool events,bool oracle,bool full){
 ...
 U selected_nodes=0;for(const auto&k:prepared)selected_nodes+=k->total_nodes;p::need(!full||(prepared.size()==1138&&model.available.size()==1138&&model.prefix==0&&!eager&&events),"full workflow requires all1138 in sealed actual order, full grids and event mode");if(native_writer_control::natural_d1){
 p::need(!validate&&!full&&!eager&&events&&model.prefix==0&&prepared.size()==773&&model.available.size()==1138,"passive D1 diagnostic requires exact natural773/full grids");
 for(std::size_t i=0;i<prepared.size();++i){const std::string expected=i<408?"epoch-1-launch-"+std::to_string(i):"epoch-2-launch-"+std::to_string(i-408);p::need(prepared[i]->call.at("source_launch_key")==expected,"diagnostic must preserve complete P408 then D1365 order");}
 p::need(selected_nodes==2715280277ULL,"diagnostic exact logical work");
 U selected_ctas=0;for(const auto&k:prepared)selected_ctas+=k->executed;p::need(selected_ctas==410363,"diagnostic exact CTA work");
 }
 if(native_writer_control::full_d2){
 p::need(full&&!validate&&!eager&&events&&model.prefix==0&&prepared.size()==1138&&model.available.size()==1138,"full EF diagnostic requires exact1138/full grids");
 for(std::size_t i=0;i<prepared.size();++i){const std::string expected=i<408?"epoch-1-launch-"+std::to_string(i):i<773?"epoch-2-launch-"+std::to_string(i-408):"epoch-3-launch-"+std::to_string(i-773);p::need(prepared[i]->call.at("source_launch_key")==expected,"full EF must preserve P408 D1365 D2365 order");}
 p::need(selected_nodes==4712768255ULL,"full EF exact logical work");U selected_ctas=0;for(const auto&k:prepared)selected_ctas+=k->executed;p::need(selected_ctas==772512,"full EF exact CTA work");
 }
 p::need(validate||full||native_writer_control::natural_d1||(prepared.size()<=64&&selected_nodes<=15000000),"bounded integration cap without explicit qualified workflow mode");
 const auto max_cycles=g::Cycle(p::natural(in.at("max_kernel_cycles"),2000000000));p::need(max_cycles>0,"positive max cycles");for(const auto&k:prepared)if(is_helper(k->family)){const auto&h=model.helpers->at(k->call.at("source_launch_key"));p::need(k->total_nodes==h.census(k->executed).nodes&&k->warps==h.warps&&k->resident==h.resident&&k->expected_read==h.census(k->executed).read&&k->expected_write==h.census(k->executed).write,"Helper Prepared exact typed model/resource closure");}auto cfg=g::make_rtx4000_ada_footprint_reference_config();cfg.per_sm_l1.sector_validity=shared_cache_control::enabled;cfg.silence_mode=true;cfg.workload_type=g::WorkloadType::Llama3Elementwise;Mapper mapper;coupling::Runtime memory(in,cfg,mapper,hbf_drain_cli::policy);p::need(bool(memory.backend),"external backend required");if(unified_trace::writer)memory.backend->set_trace_sink(unified_trace::writer);
 ...
 if(shared_cache_control::enabled){auto& profile=result["cache_configuration"];
 profile["L1"]["read_validity_policy"]="REQUESTED_SECTORS_COMPLETE_ON_DELIVERY";
 profile["L1"]["read_forwarding"]="ONE_L2_LINE_ACCESS_IF_ANY_REQUESTED_SECTOR_MISSING";
 profile["L2"]["evict_first_hit_policy"]="EMPIRICAL_FIXED_PRNG_THROTTLE_MRU_OR_LRU";
 profile["L2"]["EF_hit_rate_numerator"]=shared_cache_control::numerator;
 profile["L2"]["EF_hit_rate_denominator"]=65536;
 profile["L2"]["EF_eligibility_adapter"]="KNOWN_OBSERVED_EXACT_EF_READ_PROXY_NOT_RESOLVED_L2_DESCRIPTOR";
 profile["remaining_memgen_behavior_differences"][0]="TileGen L1 sector validity enabled; L2 remains whole-line128B fill unlike MemGen32B sectors";}
 auto shared_l2=tiny_full::make_l2(cfg,memory);if(shared_cache_control::enabled)shared_l2->enable_shared_cache(shared_cache_control::numerator);g::Cycle shared_cycle=0;hybrid_full::FineContext session(cfg,*shared_l2,shared_cycle);hybrid_full::TinyContext tiny_context(cfg,*shared_l2,memory,shared_cycle);hybrid_full::Bindings tiny_bindings(model,frames,*memory.mapper);hybrid_full::options.validate(model);U tiny_call_count=0;J routes=J::array();const double model_setup_seconds=std::chrono::duration<double>(Clock::now()-start).count();frame_bridge::Active<GemmCatalog>active([&]{return session.is_quiescent();});J frame_validation=J::array();std::weak_ptr<const GemmCatalog>prior_owner;U frozen_encoded_bytes=frames.receipt().at("encoded_payload_bytes").get<U>();const auto*session_l2=&session.l2();const auto*backend_identity=memory.backend.get();
...
tiny_runtime::sp_event_mode=events;const auto initial=session.statistics();g::Cycle previous=0,sum_windows=0,sum_kernel=0,sum_drain=0;U sum_r=0,sum_w=0,prior_r=0,prior_w=0;tiny_sha::Sha256 workflow_digest;double engine_total=0;
 std::unique_ptr<decode_gate_joint::Observation> gate_joint_observation;
 if(!native_writer_control::full_d2)gate_joint_observation=std::make_unique<decode_gate_joint::Observation>();
 decode_writer::Observation writer_observation(408,native_writer_control::full_d2?1137:772,true,gate_joint_observation.get());
 std::unique_ptr<decode_writer::Scope> writer_scope;
 g::ef_insertion::Observation insertion_observation;std::unique_ptr<g::ef_insertion::Scope> insertion_scope;g::Cycle policy_entry_cycle=-1;
 J shared_calls=J::array();
 auto execute_one=[&](std::size_t index){memory.backend->set_trace_context(index);const auto& k=*prepared[index];std::cerr<<J({{"progress_started_call",index+1},{"progress_completed_calls",index},{"total_calls",prepared.size()},{"family",k.family},{"source_launch_key",k.call.at("source_launch_key")},{"total_CTAs",k.executed},{"continuous_cycle",session.cycle()}}).dump()<<std::endl;
  ...
  // Bypass totals include policy-bypassed stores as well as explicit copy reads.
  const U policy_bypassed_stores=cfg.per_sm_l1.store_bypass?d.at("pre_l1_writes").get<U>():0;
  if(cfg.per_sm_l1.store_bypass)p::need(d.at("l1_write_hits")==0&&d.at("l1_write_misses")==0,"store bypass must not touch L1");
  if(pb){p::need(pb->copy_done==2576*k.executed&&pb->zero_done==16*k.executed&&pb->tensor_work==33554432*k.executed,"P28 copy/tensor census");p::need(d.at("pre_l1_reads")==10240*k.executed&&d.at("l1_bypassed_transactions")==10240*k.executed+policy_bypassed_stores&&d.at("l1_read_hits")==0&&d.at("l1_read_misses")==0,"P28 known bypass per-call delta");e["P28_node_audit"]=pb->receipt();}
  if(gb){const auto&m=*lease->nextmodel;p::need(gb->copy_done==m.copies*k.executed&&gb->zero_done==m.zero_copies*k.executed&&gb->tensor_work==m.tensors*2048*k.executed,"new GEMM per-CTA work");U lines=k.family=="GEMMGate"?20480:k.family=="GEMMDown"?35840:10240;p::need(d.at("pre_l1_reads")==lines*k.executed&&d.at("l1_bypassed_transactions")==lines*k.executed+policy_bypassed_stores&&d.at("l1_read_hits")==0&&d.at("l1_read_misses")==0,"new GEMM known BYPASS counters");e["GEMM_node_audit"]=gb->receipt();}
  if(ab){const auto&m=*lease->attentionmodel;p::need(ab->copy_done==m.copies*k.executed&&ab->zero_done==m.zero_copies*k.executed&&ab->tensor_work==m.tensors*2048*k.executed,"Attention copy/shared/tensor work");p::need(d.at("pre_l1_reads")==m.read_lines*k.executed&&d.at("pre_l1_writes")==m.write_lines*k.executed&&d.at("l1_bypassed_transactions")==m.copy_lines*k.executed+policy_bypassed_stores,"Attention ordinary global and known copy bypass census");e["Attention_node_audit"]=ab->receipt();}
 ...
 };for(std::size_t index=0;index<prepared.size();++index){try{
 const J shared_before=shared_cache_control::enabled?shared_l2->shared_cache_report<J>():J();
 const J shared_l1_before=shared_cache_control::enabled?shared_cache_control::counters(shared_l2->per_sm_l1_statistics()):J();
 const J shared_ready_before=shared_cache_control::enabled?shared_cache_control::readiness(shared_l2->per_sm_l1_readiness()):J();if(native_writer_control::active()&&index==408){p::need(session.is_quiescent(),"D1 entry quiescence");writer_scope=std::make_unique<decode_writer::Scope>(writer_observation,&session.l2());session.l2().decode_writer_begin();if(ef_natural_control::requested){p::need(!g::ef_insertion::active.owner&&ef_natural_control::zero(insertion_observation),"Prefill policy remained fully OFF");policy_entry_cycle=shared_cycle;insertion_scope=std::make_unique<g::ef_insertion::Scope>(insertion_observation,shared_l2.get(),true);}}
 if(native_writer_control::active()&&index>=408)writer_observation.set_call(index);
 if(index<408)p::need(!g::ef_insertion::active.owner&&ef_natural_control::zero(insertion_observation),"Prefill has no active policy or policy events");const auto hbf_before=memory.backend->service_diagnostics();g::source_service::CallSummary source_summary(index);g::source_service::Scope source_scope(source_summary,shared_l2.get(),index);execute_one(index);source_scope.close();if(shared_cache_control::enabled){
 p::need(session.is_quiescent()&&shared_l2->per_sm_l1_readiness().live_tickets==0,"shared cache call drained");
 const auto after=shared_l2->shared_cache_report<J>();const auto l1after=shared_cache_control::counters(shared_l2->per_sm_l1_statistics());
 shared_calls.push_back({{"call_index",index},{"source_launch_key",prepared[index]->call.at("source_launch_key")},
 {"before",shared_before},{"after",after},{"delta",shared_cache_control::delta(after,shared_before)},
 {"l1_before",shared_l1_before},{"l1_after",l1after},{"l1_delta",shared_cache_control::delta(l1after,shared_l1_before)},
 {"readiness_before",shared_ready_before},{"readiness_after",shared_cache_control::readiness(shared_l2->per_sm_l1_readiness())}});}result["source_transport_observation"].push_back(source_summary.report<J>(prepared[index]->call.at("source_launch_key")));result["pipeline"][index]["execution"]["hbf_service"]={{"before",coupling::native_service_statistics(hbf_before)},{"after",coupling::native_service_statistics(memory.backend->service_diagnostics())}};hybrid_full::emit_completed_summary(result["pipeline"][index],index+1);}catch(const std::exception&e){throw std::runtime_error("Execute "+prepared[index]->family+" "+prepared[index]->call.at("source_launch_key").get<std::string>()+": "+e.what());}}
 if(shared_cache_control::enabled){p::need(shared_calls.size()==1138&&session.is_quiescent()&&shared_l2->shared_cache_enabled()&&!gate_joint_observation,"complete shared full1138 without reset");
 result["shared_cache_policy"]={{"schema","SHARED_FIXED_H_RAW_EF_PROXY_FULL1138_V1"},{"enabled",true},{"model_behavior_changed",true},
 {"first_call",0},{"last_call",1137},{"call_count",1138},{"numerator",shared_cache_control::numerator},{"denominator",65536},
 {"algorithm","xorshift64star_v1_top16_threshold"},{"fixed_seed_hex","0x9e3779b97f4a7c15"},{"selector_lifetime","single_L2_owner_all_calls_all_groups_no_reseed"},
 {"eligibility","actual_resident_HIT_Known_observed_LDG.E.EF.U16_read"},{"semantic_priority_recovered",false},{"resolved_L1_and_explicit_L2","Unknown"},
 {"nonpromoted_EF_hit","active_touch_to_LRU"},{"fill_rule","all_processed_MSHR_waiters_Known_observed_ExactEF_reads_new_tag_LRU_else_MRU"},
 {"pending_merges_draw_selector",false},{"existing_line_fill_rule","original_MRU_no_selector"},{"default_epoch_order","C;R;Retry;Q"},
 {"prefill_policy_active",true},{"L1_validity_bytes",32},{"L2_validity_bytes",128},{"writeback_bytes",32},
 {"cache_reset",false},{"final_dirty_flush",false},{"hardware_policy_qualified",false},{"hardware_gap_fixed",false},
 {"calls",shared_calls},{"final_counters",shared_l2->shared_cache_report<J>()},{"end_live_tickets",shared_l2->per_sm_l1_readiness().live_tickets},
 {"layout",{{"sizeof_ReadFillTicket",sizeof(g::ReadFillTicket)},{"sizeof_ReadCompletion",sizeof(g::L2Cache::ReadCompletion)},
 {"sizeof_Transaction",sizeof(g::L2Cache::Transaction)},{"sizeof_MSHREntry",sizeof(g::L2Cache::MSHREntry)},{"sizeof_CacheLineState",sizeof(g::L2Cache::CacheLineState)},
 {"alignof_ReadFillTicket",alignof(g::ReadFillTicket)},{"alignof_Transaction",alignof(g::L2Cache::Transaction)}}}};}
 if(native_writer_control::active()){if(ef_natural_control::requested){p::need(bool(insertion_scope)&&policy_entry_cycle>=0,"D1 policy scope must have started");insertion_scope->close();result["ef_insertion_observation"]=ef_natural_control::counters<J>(insertion_observation);result[native_writer_control::full_d2?"ef_full_policy":"ef_natural_policy"]=ef_natural_control::description<J>(policy_entry_cycle,shared_cycle,native_writer_control::full_d2);}session.l2().decode_writer_end();result["decode_writer_observation"]=writer_observation.report<J>();if(gate_joint_observation)result["gate_joint_observation"]=gate_joint_observation->report<J>();writer_scope.reset();result["diagnostic_scope"]={{"name",native_writer_control::full_d2?"COMPLETE_PREFILL408_PLUS_DECODE1_365_PLUS_DECODE2_365":"NATURAL_PREFILL408_PLUS_COMPLETE_DECODE1_365"},{"full_P32_D2_result",native_writer_control::full_d2},{"omitted_suffix_calls",native_writer_control::full_d2?0:365},{"initial_history","original modeled cold Prefill; not hardware warmup"}};}
...
int main(int argc,char**argv) {
 ...
 try {
  ...
  for(int i=1;i<argc;++i) {
   ...
   else if(a=="--diagnostic-natural-d1-writer")native_writer_control::natural_d1=true;
   else if(a=="--diagnostic-full-d2-writer")native_writer_control::full_d2=true;
   else if(a=="--ef-insert-lru")ef_natural_control::requested=true;
   else if(a=="--shared-cache=h288"||a=="--shared-cache=h0"||a=="--shared-cache=h65536"){
    native_program::need(!shared_cache_control::enabled,"one shared cache selection");shared_cache_control::enabled=true;
    shared_cache_control::numerator=a=="--shared-cache=h0"?0:a=="--shared-cache=h65536"?65536:288;
   }
  ...
  native_program::need(!native_writer_control::natural_d1||(mode=="cosim-fast"&&!full&&!validate&&!eager&&events&&trace_path.empty()&&!phase_ctas),"diagnostic mode must preserve frozen fast execution with no trace");
  if(native_writer_control::natural_d1){const auto&o=hybrid_full::options;native_program::need(!o.all_fine&&!o.silu_q1&&o.q==16&&o.phase_width==16&&o.memory_epoch==8&&o.prefill_compute_width==1&&o.fine_keys.empty()&&o.fine_families.empty()&&hbf_drain_cli::policy.drain==sg_hbf::DrainMode::Independent&&hbf_drain_cli::policy.quantum==1,"diagnostic frozen fast parameters; no overrides");}
  native_program::need(!(native_writer_control::natural_d1&&native_writer_control::full_d2),"natural773 and full1138 diagnostic modes are exclusive");
  if(native_writer_control::full_d2){const auto&o=hybrid_full::options;native_program::need(full&&(ef_natural_control::requested||shared_cache_control::enabled)&&TILEGEN_DIRTY_SECTOR_MODE==2&&TILEGEN_SOURCE_MEMORY_SEMANTICS==1&&TILEGEN_EF_INSERTION_COUNTERFACTUAL==1&&mode=="cosim-fast"&&!validate&&!eager&&events&&trace_path.empty()&&!phase_ctas&&!o.all_fine&&!o.silu_q1&&o.q==16&&o.phase_width==16&&o.memory_epoch==8&&o.prefill_compute_width==1&&o.fine_keys.empty()&&o.fine_families.empty()&&hbf_drain_cli::policy.drain==sg_hbf::DrainMode::Independent&&hbf_drain_cli::policy.quantum==1,"full EF only fixed1138 original q8 and independent HBF profile");}
  native_program::need(!ef_natural_control::requested||((native_writer_control::natural_d1&&!full||native_writer_control::full_d2&&full)&&mode=="cosim-fast"&&!validate&&!eager&&events&&trace_path.empty()&&!phase_ctas),"fixed EF policy only exact natural773 or full1138 cosim-fast no trace");
  native_program::need(!shared_cache_control::enabled||(full&&native_writer_control::full_d2&&!native_writer_control::natural_d1&&!ef_natural_control::requested),"shared h profile requires its own exact full1138 mode, no old EF alias");
