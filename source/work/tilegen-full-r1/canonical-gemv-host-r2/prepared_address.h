...
struct PreparedAddress {
 ...
 // The independently validated typed address domain uses exactly the same
 // native Program::address and offset/range checks as the old constructor.
 explicit PreparedAddress(const typed::Binding& binding):program(binding.source().program){
  bindings.reserve(binding.objects().size());
  for(const auto& object:binding.objects())bindings.push_back({object.source_base,object.target_base,object.target_bytes});
 }
