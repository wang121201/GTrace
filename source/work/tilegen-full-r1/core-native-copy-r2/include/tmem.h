...
public:
    ...
    bool can_issue(const DAGNode* node, Cycle current_cycle) {
    ...
    void on_issue(const DAGNode* node, Cycle current_cycle) {
        if (!node || !is_tmem_op(node)) {
            return;
        }
        if (node->warp_id >= 0 &&
...
const CtaSparseSlots<Cycle>& cta_warp_slots() const { return next_issue_cycle; }
    void cta_erase_warp_slot(int w) { next_issue_cycle.erase_slot(w); }
private:
    struct PairKey {
        int cta_group_id;
        int mma_seq;
    ...
    bool paired_mma_issue_ready(const DAGNode* node, Cycle current_cycle) {
        ...
        if (state.node_by_rank[0] >= 0 && state.node_by_rank[1] >= 0) {
            ...
            Cycle grant_cycle = current_cycle + 1;
    ...
    CtaSparseSlots<Cycle> next_issue_cycle;
    ...
    inline static std::unordered_map<int, Cycle> node_issue_grant_cycle{};
