...
public:
 explicit Observation(int first,int last,bool unique_history=false,decode_gate_joint::Observation* joint=nullptr):first_(first),last_(last),unique_enabled_(unique_history),joint_(joint){need(first>=0&&last>=first&&last-first<1138,"writer fixed call domain invalid");calls_.resize(last-first+1);}
