...
class SM {
public:
    std::function<void(int)> cta_materialize;
    std::function<void(int,Cycle)> cta_retire;
    std::function<void(int)> cta_node_complete; // Notification after original count decrement.
    bool event_mode = false;
    std::uint64_t host_service_calls = 0;
    std::function<void(Cycle)> event_owner_wakeup;
    Cycle next_event_cycle(Cycle current_cycle) const;
    int sm_id;
    ...
    std::deque<Cycle> tb_dispatch_ready;             // Ready cycles for dispatching waiting TBs
    ...
    CtaSparseSlots<int> node_id_to_tb_id;             // Direct mapping: node ID → TB ID (O(1) lookup)
    ...
    SM(int id, int warp_count, int tensor_throughput, int simd_throughput,
       ...
       int block_schedule_latency_cycles)
        ...
        tma = new BulkCopyUnit(bulk_copy_config, id);
    ...
    bool dispatch_next_tb(Cycle current_cycle);
    ...
    void step(Cycle current_cycle, L2Cache* l2,
