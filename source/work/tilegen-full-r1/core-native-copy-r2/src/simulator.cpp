...
void Simulator::print_configuration(bool verbose) {
    ...
    *output_stream << "Max concurrent blocks per SM: " << config.max_concurrent_blocks_per_sm << "\n";
    ...
    switch (config.workload_type) {
        ...
        case WorkloadType::Llama3Elementwise: workload_name = "Llama3Elementwise"; break;
    }
...
bool Simulator::run(Cycle max_cycles, bool verbose) {
    ...
    while (cycle < max_cycles) {
        ...
        if (completed_node_count == (dag->cta_graph_store ? dag->cta_graph_store->node_count() : dag->nodes.size())) {
...
void Simulator::print_results() {
    if (dag->cta_graph_store) return; // Parent audits each CTA before reclamation.
        ...
        for (int warp_id = 0; warp_id <= max_warp_id; ++warp_id) {
            ...
            std::sort(warp_nodes.begin(), warp_nodes.end(),
                      [](const DAGNode* a, const DAGNode* b) {
                          Cycle a_issue = (a->issue_cycle >= 0) ? a->issue_cycle
                                                              : std::numeric_limits<Cycle>::max();
                          Cycle b_issue = (b->issue_cycle >= 0) ? b->issue_cycle
                                                              : std::numeric_limits<Cycle>::max();
            ...
            *output_stream << "\n--- Warp " << warp_id << " (" << warp_nodes.size() << " nodes) ---\n";
            ...
            for (auto* node : warp_nodes) {
                Cycle duration = (node->end != -1 && node->start != -1) ? (node->end - node->start) : -1;
    ...
    Cycle kernel_start = std::numeric_limits<Cycle>::max();
    Cycle last_tma_issue = std::numeric_limits<Cycle>::min();
    Cycle last_output_store_issue = std::numeric_limits<Cycle>::min();
    Cycle last_dram_store_issue = std::numeric_limits<Cycle>::min();
    for (auto* node : dag->nodes) {
        if (node->start >= 0) {
    ...
    Cycle total_cycles = cycle;
    std::string kernel_time_line;
    if (last_tma_issue != std::numeric_limits<Cycle>::min()) {
    ...
    struct TBStats {
        ...
        int finished_count;
        int first_sm_id;
        Cycle first_wgmma_issue;
        Cycle first_st_start;
        Cycle last_st_end;
        Cycle first_div_start;
        Cycle last_div_end;
        Cycle first_tma_store_start;
        Cycle last_tma_store_end;
        TBStats()
            : min_start(std::numeric_limits<Cycle>::max()),
              max_end(std::numeric_limits<Cycle>::min()),
              node_count(0),
              finished_count(0),
              first_sm_id(-1),
              first_wgmma_issue(std::numeric_limits<Cycle>::max()),
              first_st_start(std::numeric_limits<Cycle>::max()),
              last_st_end(std::numeric_limits<Cycle>::min()),
              first_div_start(std::numeric_limits<Cycle>::max()),
              last_div_end(std::numeric_limits<Cycle>::min()),
              first_tma_store_start(std::numeric_limits<Cycle>::max()),
              last_tma_store_end(std::numeric_limits<Cycle>::min()) {}
    };
    struct TileKey {
        int tb_id;
        int tile_id;
    ...
    struct RoundComputeStats {
        Cycle min_load_start;
        Cycle max_mma_end;
        RoundComputeStats()
            : min_load_start(std::numeric_limits<Cycle>::max()),
              max_mma_end(std::numeric_limits<Cycle>::min()) {}
    };
    ...
    if (config.workload_type == WorkloadType::GEMM) {
        ...
        if (!tile_stats.empty()) {
            ...
            *output_stream << std::string(140, '-') << "\n";
            ...
            for (const auto& key : keys) {
                ...
                Cycle latency = (stats.min_start != std::numeric_limits<Cycle>::max() &&
                               stats.max_end != std::numeric_limits<Cycle>::min())
                                  ? (stats.max_end - stats.min_start)
                                  : -1;
                Cycle pre_st = (stats.min_start != std::numeric_limits<Cycle>::max() &&
                              stats.first_st_start != std::numeric_limits<Cycle>::max())
                                 ? (stats.first_st_start - stats.min_start)
                                 : -1;
                Cycle st_span = (stats.first_st_start != std::numeric_limits<Cycle>::max() &&
                               stats.last_st_end != std::numeric_limits<Cycle>::min())
                                  ? (stats.last_st_end - stats.first_st_start)
                                  : -1;
                Cycle store_span = (stats.first_tma_store_start != std::numeric_limits<Cycle>::max() &&
                                  stats.last_tma_store_end != std::numeric_limits<Cycle>::min())
                ...
                if (cp_it != tile_cp_rounds.end()) {
                    ...
                    for (const auto& [_, round] : cp_it->second) {
                        if (round.min_start != std::numeric_limits<Cycle>::max() &&
                            round.max_end != std::numeric_limits<Cycle>::min()) {
                ...
                if (comp_it != tile_compute_rounds.end()) {
                    ...
                    for (const auto& [_, round] : comp_it->second) {
                        if (round.min_load_start != std::numeric_limits<Cycle>::max() &&
                            round.max_mma_end != std::numeric_limits<Cycle>::min()) {
            ...
            *output_stream << std::left << std::setw(8) << key.tb_id
                      ...
                      << std::setw(10) << (stats.min_start == std::numeric_limits<Cycle>::max() ? -1 : stats.min_start)
                      << std::setw(10) << (stats.max_end == std::numeric_limits<Cycle>::min() ? -1 : stats.max_end)
                      << std::setw(10) << latency
                          << std::setw(12) << pre_st
                          << std::setw(10) << st_span
                          ...
                          << std::setw(12) << (stats.first_wgmma_issue == std::numeric_limits<Cycle>::max()
                                                  ? -1
                                                  : stats.first_wgmma_issue)
        ...
        if (!tb_stats.empty()) {
            ...
            *output_stream << std::string(168, '-') << "\n";
            ...
            for (const auto& key : keys) {
                ...
                Cycle loop_start = std::numeric_limits<Cycle>::max();
                ...
                if (loop_start == std::numeric_limits<Cycle>::max()) {
                    loop_start = -1;
                }
                Cycle loop_end = std::numeric_limits<Cycle>::min();
                auto le_it = tb_cons_loop_end_rounds.find(key.tb_id);
                if (le_it != tb_cons_loop_end_rounds.end()) {
                    for (const auto& [_, round] : le_it->second) {
                        if (round.max_end != std::numeric_limits<Cycle>::min()) {
                            loop_end = std::max(loop_end, round.max_end);
                        }
                    }
                }
                if (loop_end == std::numeric_limits<Cycle>::min()) {
                    loop_end = -1;
                }
                Cycle pre_div = (stats.min_start != std::numeric_limits<Cycle>::max() &&
                               stats.first_div_start != std::numeric_limits<Cycle>::max())
                                  ? (stats.first_div_start - stats.min_start)
                                  : -1;
                Cycle div_span = (stats.first_div_start != std::numeric_limits<Cycle>::max() &&
                                stats.last_div_end != std::numeric_limits<Cycle>::min())
        ...
        for (const auto& key : reduce_order) {
            ...
            if (it != reduce_stats.end()) {
                ...
                if (stats.min_start != std::numeric_limits<Cycle>::max() &&
                    stats.max_end != std::numeric_limits<Cycle>::min()) {
        ...
        if (!tb_stats.empty()) {
            ...
            *output_stream << std::string(168, '-') << "\n";
            ...
            for (const auto& key : keys) {
                ...
                Cycle main_loop_start = std::numeric_limits<Cycle>::max();
                auto w1_it = tb_wgmma1_rounds.find(key.tb_id);
                if (w1_it != tb_wgmma1_rounds.end()) {
                    for (const auto& [_, round] : w1_it->second) {
                        if (round.min_start != std::numeric_limits<Cycle>::max()) {
                ...
                if (main_loop_start == std::numeric_limits<Cycle>::max()) {
                    main_loop_start = -1;
                }

                Cycle prenorm = (stats.min_start != std::numeric_limits<Cycle>::max() &&
                               stats.first_div_start != std::numeric_limits<Cycle>::max())
                                  ? (stats.first_div_start - stats.min_start)
                                  : -1;
                Cycle norm_span = (stats.first_div_start != std::numeric_limits<Cycle>::max() &&
                                 stats.last_div_end != std::numeric_limits<Cycle>::min())
                                    ? (stats.last_div_end - stats.first_div_start)
                                    : -1;
                Cycle st_span = (stats.first_st_start != std::numeric_limits<Cycle>::max() &&
    ...
    *output_stream << "Finished: " << finished << "/" << dag->nodes.size() << "\n";
    if (last_dram_store_issue != std::numeric_limits<Cycle>::min()) {
        *output_stream << "Kernel time (last DRAM store issue cycle): " << last_dram_store_issue << "\n";
    }
    if (last_output_store_issue != std::numeric_limits<Cycle>::min()) {
