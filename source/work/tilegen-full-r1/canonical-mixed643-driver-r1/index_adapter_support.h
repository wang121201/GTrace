#pragma once
#include "../canonical-indexput-driver-r2/modeled_builder.h"
...
struct ModeledPrepared {
 ...
 ModeledPrepared(const SourceBundle& s,const J& c,bool eager):bundle(s),program(s.program),registers(s.registers),call(c),executed(p::natural(c.at("grid")[0],64)){
  limits.max_live_nodes=5000000;const U ranges=544,frontier=std::min(executed,U(1152)),nodes=p::multiply(frontier,registers.nodes.size());
...
inline native_sequence::J index_pool_receipt(const canonical_indexput::IndexCatalog&catalog){
